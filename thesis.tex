...
According to \citet{LopezPazOquab2017}, the discriminative approach offers several key advantages: it is label-independent, model-agnostic, makes no assumptions about which features are important, and can detect complex high-dimensional, non-linear distributional shifts that evade univariate statistical tests. What remains missing is not the detection capability itself, but rather the integration of this detection with \textit{explainability}---the ability to attribute drift to specific features and map those attributions to actionable remediation strategies.

\section{Objectives}
The primary objective of this thesis is to develop \textbf{Explainable Adversarial Drift Detection (EADD)}, a framework that integrates discriminative classification, permutation-based significance testing, and SHAP-based feature attribution into a unified streaming drift detection pipeline capable of not only detecting drift but also diagnosing its root cause and prescribing remediation. Specifically, this work aims to:

\begin{enumerate}
    \item Extend the discriminative drift detection approach of D3 \cite{Gozuacik2019} with a non-linear classifier (LightGBM) and \textbf{reservoir sampling} for representative reference windows, broadening detection coverage to gradual and incremental drift patterns where linear classifiers fail.
    ...
    \item Develop a \textbf{SHAP-based root cause analysis} module that extracts feature-level drift attribution from the discriminative classifier, transforming the binary detection signal into a diagnostic report identifying \textit{which} features drive detected drift \citep{Lundberg2017}.
...
\begin{enumerate}
    \item \textbf{EADD Framework:} An integrated streaming drift detection pipeline that unifies discriminative classification (extending D3 \citep{Gozuacik2019}), permutation-based significance testing (adapting \citet{LopezPazOquab2017} for streaming contexts), and SHAP-based root cause analysis \citep{Lundberg2017} into a single coherent system. While individual components exist in isolation, no prior work has combined them into an end-to-end framework that provides statistically rigorous drift confirmation \textit{with} quantitative feature-level attribution \textit{and} automated prescriptions.
    
    \item \textbf{Root Cause Analysis for Drift Detection:} Application of TreeSHAP feature attribution to the discriminative classifier to identify which specific features drive detected drift. This addresses the ``black box'' limitation of existing detectors \citep{Gozuacik2019, Lukats2025} and bridges the gap between detection and remediation.
    ...
    \item \textbf{Benchmark Evaluation:} Comprehensive comparison with seven state-of-the-art unsupervised detectors on 11 benchmark data streams from the \citet{Lukats2025} framework, demonstrating that explainability can be achieved without sacrificing detection accuracy.
...
\hline
Governance \& Explainability Era (2024--2026) & Drift detection became a core component of AI governance. The focus shifted toward unsupervised and interpretable detection: TRIPODD \citep{TRIPODD2025} introduced feature-interaction aware hypothesis testing; CDSeer \citep{CDSeer2024} achieved label-efficient detection; and cloud platforms (Google Vertex AI, AWS SageMaker) standardised SHAP-based feature attribution monitoring \citep{TalySato2021, AWSSageMaker2025}. \\
\hline
\end{tabular}
\end{table}
...
The drift detection landscape has evolved rapidly, with several recent methods addressing interpretability, label efficiency, and high-dimensional data:

\textbf{CDSeer (2024):} \citet{CDSeer2024} proposed a model-agnostic, semi-supervised technique that reportedly improves precision by 57\% over previous state-of-the-art while using 99\% fewer labels. CDSeer demonstrates that label-efficient detection is achievable, but it does not provide feature-level attribution of drift sources.

\textbf{TRIPODD (2025):} \citet{TRIPODD2025} introduced a holistic framework for interpretable model drift detection that leverages feature-interaction aware hypothesis testing to identify which specific inputs are causing the drift. TRIPODD represents an important step toward interpretable drift detection. However, it focuses on identifying drifted feature interactions through statistical testing rather than providing quantitative feature importance scores or automated remediation prescriptions.

\textbf{DriftLens (2024):} \citet{DriftLens2024} designed a detector specifically for unstructured data (text, images, speech) that monitors the internal representations of deep learning models to detect changes in real time. DriftLens addresses the high-dimensional unstructured data gap but does not provide feature-level diagnostics for tabular data streams.

\textbf{STUDD (2022):} \citet{STUDD2022} proposed a student--teacher method for unsupervised concept drift detection, where a student model's divergence from a teacher model signals drift. STUDD is unsupervised but does not explain which features are responsible for the detected divergence.

\textbf{ADDM (2022):} \citet{ADDM2022} introduced an autoregressive-based drift detection method that monitors error rates and computes confidence intervals for drift points, outperforming traditional benchmarks on both synthetic and real data. Like other error-based approaches, ADDM requires access to model performance signals and does not attribute drift to specific features.

\textbf{Key Observation:} While these recent methods have advanced detection accuracy, label efficiency, and even interpretability, a gap persists in providing an integrated pipeline that combines statistically rigorous multivariate detection, quantitative feature-level attribution, and automated remediation prescriptions in a unified streaming framework.

\section{Gap in the Literature}
The preceding review identifies a progressive evolution from supervised, error-based detectors toward unsupervised, interpretable methods. Recent advances---including TRIPODD's feature-interaction hypothesis testing \citep{TRIPODD2025} and CDSeer's label-efficient detection \citep{CDSeer2024}---have begun to address interpretability and practicality. Nevertheless, several critical gaps remain that limit practical utility in production MLOps environments:

\begin{enumerate}
    \item \textbf{The ``Black Box'' Problem:} According to \citet{Lukats2025}, most existing detectors---including the benchmark-leading D3---signal \textit{that} drift occurred but provide no insight into \textit{which} features drifted or \textit{why}. While TRIPODD \citep{TRIPODD2025} addresses this partially through feature-interaction testing, it does not produce quantitative importance scores suitable for automated decision-making.
    
    \item \textbf{No Root Cause Analysis with Quantitative Attribution:} Current methods provide either a binary detection signal \citep{Lukats2025, Gemaque2020} or qualitative interaction-level diagnostics \citep{TRIPODD2025}. Production systems require quantitative, feature-level attribution that pinpoints drift sources and enables automated downstream responses.
    ...
    \item \textbf{No Actionable Prescriptions:} Even when drift is detected and partially explained, no existing method maps its diagnostics to actionable remediation strategies---whether to retrain fully, adjust specific features, or apply targeted corrections \citep{Bayram2022, Ditzler2015}. The decision of \textit{how to respond} remains entirely manual.
\end{enumerate}

This thesis addresses these gaps by proposing \textbf{Explainable Adversarial Drift Detection (EADD)}, which integrates discriminative classification with permutation-based significance testing (adapting the C2ST framework of \citealt{LopezPazOquab2017} for streaming contexts), SHAP-based root cause analysis \citep{Lundberg2017}, and automated remediation prescriptions---transforming drift detection from a binary alarm into an intelligent diagnostic tool. The contribution is not the discriminative detection mechanism itself (established by D3 and C2ST), but rather the \textbf{end-to-end diagnostic pipeline} that bridges detection, statistical validation, quantitative feature attribution, and automated prescription in a single streaming framework.
...
\begin{figure}[!htpb]
    ...
    \end{tikzpicture}
    \caption{Overview of Explainable Adversarial Drift Detection (EADD) methodology. Blue boxes indicate EADD-specific extensions beyond D3: Permutation Test (Step~3, adapting \citealt{LopezPazOquab2017} for streaming detection) and SHAP Feature Attribution (Step~4). The framework integrates discriminative classification with statistical rigor and root cause analysis.}
    \label{fig:methodology-overview}
\end{figure}

\section{Overview: Explainable Adversarial Drift Detection (EADD)}
This thesis proposes \textbf{Explainable Adversarial Drift Detection (EADD)}, a framework designed specifically for \textbf{Feature Drift} ($P(X)$ shift). The fundamental insight is that \textbf{discriminative classification} and \textbf{Feature Drift Detection} are mathematically equivalent---both ask: \textit{``Can a model distinguish between old data and new data based solely on features?''}

While existing methods like D3 \citep{Gozuacik2019} successfully employ this discriminative principle to flag distributional changes, and \citet{LopezPazOquab2017} have established the theoretical framework for classifier-based two-sample testing with significance calibration, no prior work has combined these established techniques with post-hoc feature attribution and automated prescriptions to create an end-to-end explainable drift detection pipeline for streaming data.
...
\begin{enumerate}
    ...
    \item \textbf{SHAP-Based Root Cause Analysis:} Extracting feature-level drift attribution from the discriminative classifier via TreeSHAP \citep{Lundberg2017}, transforming the binary detection signal into a diagnostic report
...
\hline
``Discriminative Detection'' & Step 2: Train classifier to distinguish $W_{ref}$ vs $W_{cur}$ \\
``Statistical Rigor'' & Step 3: Permutation test ($B{=}50$, $\alpha{=}0.01$) for principled drift confirmation \\
``Extends D3'' & Foundation: Same discriminative logic, upgraded classifier (LightGBM) \\
``Explainability'' & Step 4: SHAP identifies which $P(X)$ components shifted \\
``Actionable Prescriptions'' & Maps SHAP patterns to MLOps remediation strategies \\
\hline
\end{tabular}
\end{table}
...
\textbf{Step 2: Discriminative Classification (Detector)} --- Train discriminative classifier on $W_{ref}$ vs $W_{cur}$
...
\textbf{Benefit:} This prevents false alarms caused by small sample sizes or random fluctuations---a common failure mode in standard discriminative detection. The permutation test provides statistical guarantees that the detected drift is real, not noise.
...
This thesis addressed a critical gap in modern drift detection: the lack of explainability. While state-of-the-art methods like D3 \citep{Gozuacik2019} have demonstrated that discriminative classification can accurately detect distribution changes, and \citet{LopezPazOquab2017} have established principled significance testing for classifier-based two-sample tests, no prior work has integrated these capabilities with quantitative feature-level attribution and automated remediation prescriptions in a unified streaming framework.
...
\begin{enumerate}
    ...
    \item \textbf{Permutation Testing:} Adapting the classifier two-sample test framework of \citet{LopezPazOquab2017} for streaming drift detection, replacing D3's fixed AUC threshold with a statistically rigorous p-value ($p < 0.01$) that substantially reduced false alarms---EADD produced a mean of only 3.2 false alarms across all stable stream types (zero on three of four types) including challenging autocorrelated data where D3 produced up to 86.4 false alarms (Experiment 4).
...
The transition from opaque detection to explainable diagnostics represents an important direction for MLOps. The experimental results demonstrate that this transition does not sacrifice detection accuracy---in fact, EADD's design innovations (reservoir sampling and permutation testing) enabled complete drift coverage (4/4 types) with near-zero false alarms, a combination no other detector achieved.
...
The near-elimination of false alarms is particularly significant for production deployments: each false alarm triggers unnecessary model retraining, consuming computational resources and eroding operator trust. EADD's permutation test addresses a key pain point in industrial ML monitoring, especially for temporally correlated data streams where threshold-based detectors like D3 are unreliable.
