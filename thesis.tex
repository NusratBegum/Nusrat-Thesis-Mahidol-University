...
\begin{enumerate}
    ...
    \item Introduce \textbf{prediction entropy monitoring} ($H_{\text{pred}}$), a continuous uncertainty monitoring signal derived from the Shannon entropy \citep{Shannon1948} of the discriminative classifier's output probabilities, providing sub-threshold trend information on every monitoring step---including steps where AUC has not yet reached the detection threshold.

    \item Develop an \textbf{adaptive sequential permutation test (ASPT)} that calibrates the number of permutations based on accumulated evidence strength, reducing the permutation budget by 90--95\% in practice compared to fixed-budget approaches while maintaining Type~I error control at the nominal level \citep{Gandy2009}.

    \item Apply \textbf{SHAP-based drift localisation} using TreeSHAP \citep{Lundberg2017} on the trained LightGBM discriminator to identify which features drive detected drift, with SHAP interaction values \citep{LundbergInteraction2020} reported as a supplementary diagnostic for correlated feature subspaces (future work: formal statistical validation of interaction attributions).
...
This thesis makes the following three contributions. The six objectives above map to contributions as follows: Objectives~2--3 → C1; Objectives~4--5 → C2; Objective~6 → C3. Objective~1 (LightGBM with reservoir sampling) is a design choice enabling all three contributions rather than a standalone contribution.

\begin{enumerate}
    \item Inspired by the statistical analysis of the classifier two-sample test \citep{LopezPazOquab2017} and its extensions \citep{EC2ST2024}, this thesis introduces an \textbf{Adaptive Sequential Permutation Test (ASPT)} \citep{Gandy2009} with accompanying \textbf{prediction entropy monitoring} ($H_{\text{pred}}$). ASPT monitors evidence accumulation and terminates when statistical significance is established or refuted, reducing the permutation budget by 90--95\% in practice compared to fixed-budget permutation testing while preserving Type~I error guarantees. $H_{\text{pred}}$ provides a continuous uncertainty monitoring signal from the Shannon entropy of the discriminative classifier's output probabilities, delivering sub-threshold trend information on every monitoring step---not only when AUC reaches the detection threshold (Chapter~\ref{chap:methodology}, Steps~2b--3).

    \item Building on SHAP feature distribution analysis \citep{Lundberg2017}, this thesis proposes \textbf{SHAP-based drift localisation} and a novel \textbf{Drift Severity Index (DSI)}. The localisation component uses TreeSHAP marginal attributions to identify which features drive detected drift, with SHAP interaction values \citep{LundbergInteraction2020} as a supplementary diagnostic. DSI is defined as $\text{DSI} = \text{AUC} \times (1 - H_{\text{norm}}(\mathbf{s}))$, combining detection confidence with attribution concentration into a continuous, theoretically motivated severity score (Chapter~\ref{chap:methodology}, Step~4).

    \item A \textbf{comprehensive experimental evaluation} benchmarking EADD against seven state-of-the-art unsupervised detectors on 13 real-world benchmark streams and four synthetic drift types, with formal validation of detection coverage, false alarm control, and SHAP attribution accuracy (Chapter~\ref{chap:results}).
\end{enumerate}
...
\begin{figure}[!htpb]
    ...
    \begin{tikzpicture}[
        ...
        \node [block, right=of in] (compare) {1. Discriminative\\Comparison};
        \node [block, right=of compare] (test) {2. Statistical\\Validation};
        \node [block, below=of compare] (explain) {3. Feature\\Attribution};
        \node [block, left=of explain] (out) {Actionable\\Drift Report};
...
\begin{figure}[!htpb]
    ...
    \node[proc, below=0.72cm of dec2] (s4) {%
        \textbf{Step 4. Feature Attribution and Severity}\\[-1pt]
        {\footnotesize TreeSHAP ranking\,; $\mathrm{DSI}=\mathrm{AUC}\times(1-H_\mathrm{norm}(\mathbf{s}))$}
    };

    \node[term, below=of s4] (output) {%
        \textbf{Drift Alert Output}\\[-1pt]
        {\footnotesize uncertainty ($H_\mathrm{pred}$), significance ($p$),\\ attribution (SHAP), severity (DSI), recommended action}
    };
    ...
    \end{tikzpicture}
    \caption{Detailed architecture of the Enhanced Adversarial Drift Detection (EADD) methodology, matching the pipeline design used in the JCSEE paper: adaptive windowing, discriminative detection, ASPT significance gating, and SHAP+DSI diagnostics.}
...
\begin{figure}[!htpb]
    ...
    \begin{tikzpicture}[
        ...
        arr/.style={-{Stealth[scale=0.9]}, semithick, black!75}
    ]

    \node[box] (s0) {\textbf{Input stream and windowing:} $W_{\mathrm{ref}}$ via reservoir sampling and $W_{\mathrm{cur}}$ via sliding window};

    \node[box, below=of s0] (s1) {
        \textbf{Contribution 1: Discriminative drift signal + entropy early warning} \\
        Train C2ST classifier $f_\theta$ on $W_{\mathrm{ref}} \cup W_{\mathrm{cur}}$ and monitor \\
        discriminative AUC together with prediction entropy $H_{\mathrm{pred}}$ for early warning.
    };

    \node[box, below=of s1] (s2) {
        \textbf{Contribution 2: Adaptive Sequential Permutation Test (ASPT)} \\
        Use adaptive permutations with sequential early stopping to estimate significance; \\
        if $p\ge\alpha$, continue monitoring; if $p<\alpha$, confirm drift.
    };

    \node[box, below=of s2] (s3) {
        \textbf{Contribution 3: SHAP interaction-aware localisation} \\
        Estimate SHAP feature and interaction scores from the discriminative model to rank \\
        drifting features and identify coupled interaction-level drift drivers.
    };

    \node[box, below=of s3] (s4) {
        \textbf{Contribution 4: Drift Severity Index (DSI)} \\
        Compute $\mathrm{DSI}=\mathrm{AUC}\times(1-H_{\mathrm{norm}})$ to summarize
        concentration and operational severity of the detected drift.
    };

    \node[box, below=of s4] (s5) {\textbf{Actionable output:} drift significance, severity tier, top drifting features/interactions, and recommended MLOps response};
...
$H_{\text{pred}}$ captures distributional instabilities \textit{before} they pass the ASPT significance threshold ($p < \alpha$). A declining $H_{\text{pred}}$ trend across consecutive monitoring windows serves as a leading indicator for emerging drift, enabling proactive monitoring even when individual windows do not trigger drift alarms. This is particularly valuable for gradual and incremental drift, where the distributional change accumulates slowly. Note: $H_{\text{pred}}$'s standalone predictive value as an early-warning signal is supported by qualitative observation in the component analysis (Section~\ref{sec:component-analysis}); a dedicated quantitative evaluation---threshold selection, precision/recall of pre-alarm warnings---is left to future work.
...
\subsection{Step 4a: Feature Drift Localisation via SHAP --- Contribution 2a}
Once the ASPT confirms drift ($p < \alpha$), the next objective is to determine \textit{which features drifted}. D3 \citep{Gozuacik2019} discards the trained classifier after computing AUC---what we term the ``detect-and-discard'' pattern. However, the discriminative classifier has learned precisely which features differentiate the reference data from the current data.

The primary validated component of Contribution 2a is \textbf{per-feature SHAP attribution}, evaluated quantitatively in Experiment~3 (Section~\ref{sec:exp3-results}). SHAP interaction values are additionally computed as a \textbf{supplementary diagnostic} for correlated feature subspaces; their formal statistical accuracy is left to future work.
...
Beyond diagnosis, EADD provides \textbf{severity-proportional remediation recommendations} driven by the Drift Severity Index (Table~\ref{tab:dsi-tiers}). Unlike prior approaches that rely on fixed heuristic thresholds, DSI-based prescriptions are determined by both detection confidence (AUC) and drift concentration (normalised SHAP entropy), enabling proportional responses. The tier boundaries are proposed heuristics; empirical calibration across diverse domains is recommended before operational use:
...
\begin{itemize}
    ...
    \item \textbf{PokerHand} \citep{Lukats2024}: 100,000 samples $\times$ 10 features; card hand classification stream.
...
Five experiments were designed to systematically validate EADD's detection accuracy, SHAP attribution accuracy, and robustness:

\textbf{Experiment 1: Two-Detector Sensitivity to Temporal Drift Patterns}

Evaluated EADD's ability to detect all four major temporal drift patterns (abrupt, gradual, incremental, recurring) on synthetic data ($n=10{,}000$) with known ground-truth drift points. Compared detection delay and success rate against D3 across 5 independent runs per drift type.

\textbf{Experiment 2: Real-World Benchmark Comparison}

Benchmarked EADD against D3 across 13 real-world benchmark data streams (4 annotated + 9 unannotated), measuring Recall, MTD, and detection counts. A supplementary multi-detector comparison (EADD, D3, BNDM, CSDDM, IBDD, OCDD, SPLL, UDetect) was also conducted on an 11-stream subset to contextualise EADD's performance relative to the full Lukats et al.\ benchmark field.
...
\textbf{Experiment 5: Full Eight-Detector Synthetic Benchmark}

Compared all eight detectors (EADD, D3, BNDM, CSDDM, IBDD, OCDD, SPLL, UDetect) on the same four synthetic drift patterns at $n=5{,}000$, using the default configurations from the \citet{Lukats2024} framework. This experiment supports the statistical hypothesis tests in Section~\ref{sec:h1} and is reported there. The smaller stream length ($n=5{,}000$ vs $n=10{,}000$ in Experiment~1) matches the Lukats et al.\ benchmark protocol, enabling direct comparison against the published multi-detector baseline.
...
\textbf{Note on synthetic protocols.} Two experiments evaluate EADD on the same four drift types. Experiment~1 (Section~\ref{sec:exp1}) uses a two-detector protocol (EADD vs D3, $n{=}10{,}000$) to examine detection delay in detail. Experiment~5 (Section~\ref{sec:h1}) uses the full eight-detector field at $n{=}5{,}000$ to align with the \citet{Lukats2024} benchmark protocol for hypothesis testing. Both confirm that EADD detects all four drift types (4/4) with zero false alarms---the only detector to achieve this combination.
...
\begin{itemize}
    \item \textbf{H1 (Detection Coverage):} EADD detects a broader range of temporal drift patterns than D3. Formally: H1a evaluates drift-type coverage across all eight detectors (comparative tabulation of detected types and false alarm counts; Experiments 1 and 5), and H1b tests whether EADD achieves comparable detection delay where both detectors succeed (Mann--Whitney U test, $\alpha = 0.05$).
...
\begin{figure}[!htbp]
    ...
    \caption{Heatmaps of detection performance on synthetic drift. Top: Mean Time to Detection (lower is better; white cells indicate complete miss). Bottom: False Alarm count (lower is better).}
...
\begin{figure}[!htbp]
    ...
    \caption{Bell curves showing feature distributions before (blue) and after (red) drift injection. Top: Abrupt drift with clear separation. Bottom: Gradual drift with overlapping distributions---the subtle shift that D3, SPLL, and UDetect all failed to detect.}
...
\begin{figure}[!htbp]
    ...
    \caption{Time-series comparison of all eight detectors. Top: Abrupt drift---most detectors respond near the true drift point (dashed line). Bottom: Gradual drift---only EADD, BNDM, CSDDM, IBDD, and OCDD detect the shift; D3, SPLL, and UDetect remain silent.}
...
\textbf{Procedure:}
Thirteen real-world datasets (Section~\ref{sec:datasets}) were processed in streaming fashion by EADD and D3. For the four annotated datasets (InsectsAbrupt, InsectsGradual, InsectsIncrAbrupt, InsectsIncrRecur), Recall and Mean Time to Detection (MTD) were computed against known drift indices. For nine unannotated datasets, the total number of detections was recorded as a selectivity indicator.

A supplementary multi-detector run compared all eight detectors on an 11-stream subset from the same benchmark (excluding Luxembourg, Ozone, PokerHand, RialtoBridge). A per-detector timeout of 120 seconds was applied for this supplementary run, as BNDM's computation exceeded practical limits on 33-feature INSECTS datasets.
...
\begin{table}[!htbp]
\centering
\caption{Experiment 2: EADD vs.\ D3 on the four annotated real-world datasets. Prec.\ = Precision; Rec.\ = Recall on known drift points; MTD = Mean Time to Detection (samples; lower is better). Bold indicates EADD outperforms D3. ``---'' = all known drift points missed. Low precision on incremental datasets reflects the INSECTS annotation convention: only major transition boundaries are annotated; additional detections may correspond to unannotated continuous incremental changes rather than false alarms. Data from \texttt{experiment2\_realworld.csv}.}
\label{tab:exp2-results}
\begin{tabular}{l|c|c|c|c|c|c}
\hline
\textbf{Dataset} & \textbf{Prec.\ (EADD)} & \textbf{Rec.\ (EADD)} & \textbf{Prec.\ (D3)} & \textbf{Rec.\ (D3)} & \textbf{MTD (EADD)} & \textbf{MTD (D3)} \\
\hline
InsectsAbrupt     & 0.33 & 0.60 & 0.44 & 0.80 & \textbf{135} & 155.5 \\
InsectsGradual    & 0.00 & 0.00 & 0.00 & 0.00 & ---          & ---   \\
InsectsIncrAbrupt & 0.14 & \textbf{1.00} & 0.22 & 1.00 & \textbf{33} & 272.5 \\
InsectsIncrRecur  & 0.17 & \textbf{1.00} & 0.18 & 1.00 & \textbf{133} & 323.5 \\
\hline
\end{tabular}
\end{table}

\textbf{Multi-Detector Context (11-stream subset):}
Table~\ref{tab:exp2-multidet} presents the supplementary multi-detector comparison on an 11-stream subset, including semi-synthetic benchmarks SineClusters and WaveformDrift2 which have annotated drift points in the Lukats et al.\ framework.

\begin{table}[!htbp]
\centering
\caption{Supplementary Multi-Detector Comparison on 11-stream subset with annotated datasets. MTD = Mean Time to Detection (samples). ``---'' = detector missed all annotated drifts or timed out.}
...
Table~\ref{tab:exp2-nogt} summarizes detection activity on the nine unannotated datasets (EADD vs.\ D3).

\begin{table}[!htbp]
\centering
\caption{Experiment 2: Detection counts on nine unannotated real-world datasets (EADD vs.\ D3). Ground-truth drift counts are unknown; counts indicate detection activity, not false alarm rates. Data from \texttt{experiment2\_realworld.csv}.}
\label{tab:exp2-nogt}
\begin{tabular}{lcc}
\hline
...
PokerHand & 199 & 193 \\
Powersupply & 9 & 18 \\
RialtoBridge & 147 & 136 \\
\hline
\textbf{Total} & \textbf{467} & \textbf{467} \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[!htbp]
    ...
    \caption{Heatmaps of real-world benchmark performance. Top: Mean Time to Detection (lower is better; white cells indicate missed drifts or timeout). Bottom: Missed Detection Rate (0.0 is perfect).}
...
\begin{figure}[!htbp]
    ...
    \caption{Time-series comparison on benchmark datasets from the multi-detector supplementary run. Top: WaveformDrift2 (semi-synthetic)---EADD detects all three drift points (MTD=99), outperforming D3 (MTD=179). Bottom: SineClusters (semi-synthetic)---IBDD detects rapidly but with many excess detections, illustrating the sensitivity--specificity trade-off.}
...
\begin{figure}[!htbp]
    ...
    \caption{Feature distribution bell curves. Top: WaveformDrift2 (semi-synthetic) showing multi-dimensional distributional shift across 21 features. Bottom: InsectsAbrupt showing abrupt distributional changes across 33 features.}
...
\begin{enumerate}
    \item \textbf{EADD's Competitive Real-World Performance:} EADD detected drift on three of four annotated datasets (Table~\ref{tab:exp2-results}). EADD outperforms D3 on MTD in all three detected cases: InsectsAbrupt (MTD 135 vs.\ 155.5), InsectsIncrAbrupt (MTD 33 vs.\ 272.5, 8$\times$ faster), and InsectsIncrRecur (MTD 133 vs.\ 323.5, 2.4$\times$ faster). The MTD advantage is most pronounced on incremental drift types where D3's linear classifier fails to respond quickly. On InsectsAbrupt, EADD's lower recall (0.60 vs.\ D3's 0.80) reflects its precision-first operating point. These MTD values are single-run point estimates; variance across runs is not reported. Both EADD and D3 missed InsectsGradual---a slow gradual transition that faster-responding detectors (CSDDM MTD=143, IBDD MTD=88) detected at substantially higher false alarm cost. On the nine no-ground-truth datasets (Table~\ref{tab:exp2-nogt}), EADD and D3 produced identical total detection counts (467 each), indicating comparable selectivity.
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.78\textwidth,height=0.24\textheight,keepaspectratio]{figures/bell_shap_combined_univariate.pdf}\\[0.6em]
    \includegraphics[width=0.78\textwidth,height=0.24\textheight,keepaspectratio]{figures/bell_shap_combined_subset.pdf}\\[0.6em]
    \includegraphics[width=0.78\textwidth,height=0.24\textheight,keepaspectratio]{figures/bell_shap_combined_multivariate.pdf}
    \caption{Combined bell curve and SHAP attribution analysis for three controlled scenarios. Top: Univariate drift---F3 dominates at 52.3\%. Middle: Subset drift---F5, F2, F7 collectively account for 65\%. Bottom: Multivariate drift---importance distributed evenly, max 14.6\%.}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.86\textwidth,height=0.34\textheight,keepaspectratio]{figures/shap_attribution_accuracy.pdf}\\[0.7em]
    \includegraphics[width=0.86\textwidth,height=0.34\textheight,keepaspectratio]{figures/explainability_comparison.pdf}
    \caption{Top: SHAP attribution accuracy across all three scenarios (P@k = R@k = F1@k = 1.0 for all). Bottom: Diagnostic capability comparison---EADD is the only detector providing feature-level attribution and automated prescriptions.}
    \label{fig:shap-accuracy}
\end{figure}

\textbf{Key Findings:}
EADD achieved \textit{perfect} attribution accuracy across all three controlled synthetic scenarios with independent features: P@k = 1.0, R@k = 1.0, F1@k = 1.0. These results hold under controlled conditions; generalisation to correlated or real-world features requires additional validation. This means:
\begin{itemize}
    ...
    \item \textbf{Multivariate:} No single feature exceeded 14.6\% importance, correctly triggering the ``Multivariate Feature Shift'' prescription.
\end{itemize}
...
\textbf{ASPT Computational Savings:} The ASPT terminated early in all three scenarios, using substantially fewer than the maximum 199 permutations configured ($B_{\max} = 199$). All confirmed detections terminated at exactly $i = 20$ permutations (verified from logged \texttt{last\_aspt\_permutations}: $p = 1/21 \approx 0.048$), consistent with the 90--95\% computational reduction range reported in the methodology. The controlled drift injections (1--3$\sigma$ mean shifts) produced clear distributional separability, enabling the sequential stopping rule to confirm significance without exhausting the full permutation budget. The early termination confirms that ASPT efficiently identifies clear distributional shifts while reserving its full budget only for ambiguous cases near the decision boundary.

\section{Experiment 4: Robustness to Stable Data (False Alarm Evaluation)}
\label{sec:exp4}

\textbf{Objective:} Assess false alarm rates on stable data streams to evaluate ASPT's ability to suppress spurious detections caused by temporal autocorrelation and distributional fluctuations.

\textbf{Experimental Design:}
Four types of stable synthetic streams with zero actual drift ($n = 10{,}000$ each) were generated:
\begin{enumerate}
    \item \textbf{Gaussian i.i.d.:} Independent samples from $\mathcal{N}(0, 1)$
    \item \textbf{Autocorrelated AR(1):} $x_t = 0.8 x_{t-1} + \epsilon_t$, $\epsilon_t \sim \mathcal{N}(0, 1)$
    \item \textbf{Heteroscedastic:} Time-varying variance $\sigma^2_t$, stable mean
    \item \textbf{Correlated features} ($\rho = 0.7$): $d = 5$ features with fixed correlation structure
\end{enumerate}
Each stream was processed 5 times by both EADD and D3 at AUC thresholds $\tau \in \{0.6, 0.7, 0.8\}$.

\textbf{Results:}
EADD produced zero false alarms across all four stream types and all threshold settings. D3 was particularly vulnerable to autocorrelated data: 87.4 mean false alarms at $\tau{=}0.6$ and 54.6 at $\tau{=}0.7$. The ASPT correctly classifies temporal autocorrelation as non-significant ($p > 0.05$) because permuted AUC scores match the observed AUC under label shuffling---the permutation test accounts for whether the AUC elevation is \textit{reproducible under randomisation}, not merely whether it exceeds a fixed threshold.

\textbf{Key Finding:} The permutation test acts as a statistical ``filter'' that suppresses AUC spikes caused by autocorrelation and heteroscedasticity. D3's fixed threshold has no such filter, making it systematically vulnerable to non-stationary noise. These results are consistent with the real-world selectivity observed in Experiment~2, where EADD and D3 produced identical total detection counts (467 each) on nine unannotated stable-ish streams.
...
False alarm counts from EADD ($n=4$ stream types) and D3 at multiple thresholds were compared using one-sided Mann--Whitney U tests (from the false alarm robustness evaluation in Experiment~4, Section~\ref{sec:exp4}).
...
\textbf{Result:} Experiment 3 yielded P@k = R@k = F1@k = 1.0 across three controlled synthetic scenarios, with correct prescriptions in 3/3 cases. The evaluation covers a limited scope (n=3 scenarios, controlled shift magnitudes); broader generalisation to diverse drift patterns and real-world data requires additional validation. Accordingly, H3 is confirmed within the evaluated scope with the caveat that its generalisability is yet to be established.
...
\begin{itemize}
    ...
    \item \textbf{Real-World Data (Experiment 2):} Across 13 real-world datasets, EADD detected drift on 3 of 4 annotated INSECTS datasets, outperforming D3 on MTD in all three detected cases: InsectsAbrupt (135 vs.\ 155.5), InsectsIncrAbrupt (33 vs.\ 272.5, 8$\times$ faster), and InsectsIncrRecur (133 vs.\ 323.5; single-run estimates). On nine no-ground-truth datasets, EADD and D3 produced identical total detection counts (467 each). A supplementary multi-detector comparison confirmed BNDM timed out on all 33-feature datasets, revealing a scalability limitation.
...
\begin{itemize}
    ...
    \item \textbf{Multivariate Drift:} No single feature exceeded 14.6\% importance (AUC = 0.806), correctly triggering the ``Multivariate Feature Shift'' prescription. P@k = R@k = F1@k = 1.0.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Real-World Selectivity (Experiment 2):} On 9 no-ground-truth datasets, EADD and D3 produced identical total detection counts (467 each), indicating comparable selectivity. On the supplementary 11-stream multi-detector run, EADD's detection count was among the lowest (comparable to D3), while IBDD produced 1{,}672 and OCDD 439, consistent with their known over-detection tendencies.
...
\begin{itemize}
    ...
    \item \textbf{Orchestration \& Model Registry (e.g., MLflow / Apache Airflow):} When EADD's permutation test confirms drift ($p < 0.05$), it pushes the SHAP diagnostic report to the orchestrator. If the diagnosis is ``Multivariate Feature Shift,'' Airflow automatically triggers the training pipeline to fetch new data, retrain the primary model, and log the new artifact in MLflow. If the diagnosis is ``Univariate Drift,'' an alert is sent to the Data Engineering team via PagerDuty/Slack to inspect the specific upstream data pipeline for sensor or schema failures.
\end{itemize}

This architecture ensures that the computational overhead of the permutation test is decoupled from the primary model's inference latency, allowing EADD to run asynchronously as an independent diagnostic watchdog. \textit{Note: the DSI tier boundaries used to route automated responses are proposed heuristics not empirically validated in the reported experiments (see Limitations, Section~\ref{chap:discussion}); domain-specific calibration is required before operational deployment.}
...
\textbf{DSI Tier Calibration:} The DSI severity tier boundaries ($0.3$ and $0.6$) are \textit{proposed} heuristics; they are not empirically validated in the reported experiments (all DSI values fall in the Low tier $< 0.3$, and the Moderate and Critical tiers are never triggered). Domain-specific calibration is required before operational deployment.
...
The experimental results provide strong evidence for EADD's effectiveness across research questions. EADD is the only detector to achieve 4/4 synthetic drift-type coverage with zero false alarms (RQ1), detected drift on 3 of 4 annotated real-world datasets with competitive MTD (single-run estimates; RQ1), demonstrated perfect SHAP attribution accuracy (P@k/R@k/F1@k = 1.0) in controlled settings (RQ2), and produced zero false alarms on all stable stream types while D3 produced up to 87.4 (RQ3). Hypothesis H2 was confirmed with statistical significance at D3's $\tau=0.6$ setting ($p = 0.0101$); at the matched $\tau=0.7$ operating point the difference is directionally consistent (0 vs 54.6 false alarms) but not significant ($p = 0.2266$). The remaining detection limitation is InsectsGradual, where EADD (along with D3, BNDM, OCDD, and UDetect) missed the annotated drift point under default window configuration.
...
\begin{enumerate}
    \item Inspired by the statistical analysis of C2ST \citep{LopezPazOquab2017} and its extensions \citep{EC2ST2024}: an \textbf{Adaptive Sequential Permutation Test (ASPT)} \citep{Gandy2009} with \textbf{prediction entropy monitoring} ($H_{\text{pred}}$). ASPT monitors evidence accumulation and terminates when the decision boundary is crossed, reducing the permutation budget by 90--95\% in practice while maintaining Type~I error control. $H_{\text{pred}}$ flags emerging distributional instabilities before AUC exceeds the significance threshold.
    \item Building on SHAP feature distribution analysis \citep{Lundberg2017}: \textbf{SHAP interaction-aware drift localisation} \citep{LundbergInteraction2020} and a novel \textbf{Drift Severity Index (DSI)} ($\text{DSI} = \text{AUC} \times (1 - H_{\text{norm}}(\mathbf{s}))$). EADD correctly identified drifting features in 100\% of controlled synthetic scenarios with independent features (Experiment~3, P@k = R@k = F1@k = 1.0). DSI provides a continuous, theoretically motivated severity score whose tier boundaries are proposed heuristics requiring domain-specific calibration.
...
\begin{itemize}
    ...
    \item \textbf{Experiment 2:} In the 13-dataset real-world benchmark, EADD detected drift on 3 of 4 annotated INSECTS datasets, outperforming D3 on MTD in all three detected cases (InsectsAbrupt: 135 vs.\ 155.5; InsectsIncrAbrupt: 33 vs.\ 272.5; InsectsIncrRecur: 133 vs.\ 323.5). On nine no-ground-truth datasets, EADD and D3 produced identical total detection counts (467 each).
...
\textbf{Early Warning via Prediction Entropy ($H_{\text{pred}}$):} The prediction entropy signal tracks distributional instability before it crosses the formal significance threshold. Qualitative observation across experiments suggests that declining $H_{\text{pred}}$ trends across consecutive monitoring windows may enable proactive capacity planning and pre-emptive investigation; however, $H_{\text{pred}}$'s standalone predictive value has not been formally evaluated (threshold selection, precision/recall of pre-alarm warnings are left to future work).

\textbf{Computational Efficiency via ASPT:} The adaptive sequential permutation test reduces the mean permutation budget by 90--95\% compared to the fixed-budget baseline (averaging ${\sim}$12 of 199 maximum permutations on stable segments; exactly 20 of 199 on confirmed drift events), making permutation-based statistical calibration practical for moderate-frequency monitoring. This addresses the computational objection that has limited adoption of principled significance testing in production drift detectors.
...
\textbf{Regulatory Compliance:} In regulated sectors such as finance and healthcare, ``black box'' model adaptations may violate transparency requirements \citep{GoodmanFlaxman2017}. EADD's embedded SHAP attribution provides an automated audit trail that documents \textit{which} features drove each drift-triggered model update, supporting compliance with algorithmic fairness and transparency regulations.
...
\begin{itemize}
    ...
    \item \textbf{DSI Calibration:} The DSI severity tier boundaries ($0.3$ and $0.6$) are \textit{proposed} heuristics; they are not empirically validated in the reported experiments (all DSI values fall in the Low tier $< 0.3$, and the Moderate and Critical tiers are never triggered). Domain-specific calibration is required before operational deployment.

    \item \textbf{Classifier Ablation:} The improvement in gradual and incremental drift detection (EADD vs D3) is attributable to a combination of the LightGBM classifier, reservoir sampling, and revised window configuration. We do not isolate the contribution of each component via an ablation study. In particular, replacing D3's logistic regression with LightGBM alone (without ASPT) may already achieve 4/4 synthetic drift-type coverage, since gradient boosting captures non-linear interactions that logistic regression cannot. The benefit attributable specifically to ASPT---beyond the classifier upgrade---cannot be determined from the current experimental design.
    ...
    \item \textbf{Limited Attribution Scenarios:} The attribution accuracy validation (H3) achieved perfect P@k = R@k = F1@k = 1.0 across three controlled synthetic scenarios with independent features. This is an optimistic setting for SHAP---when the drifting feature is the only signal in an independent feature set, any rank-order attribution method will identify it. Performance on correlated, redundant, or real-world features requires separate validation.
\end{itemize}
...
This thesis has advanced the discriminative drift detection paradigm through three methodological contributions that together form an \textbf{integration framework}: each component draws on established methods (Gandy~\citep{Gandy2009} for sequential testing, TreeSHAP~\citep{Lundberg2017,LundbergInteraction2020} for attribution), but their combination within a single unsupervised streaming pipeline is non-trivial---the AUC gate must precede ASPT to maintain conservative Type~I error, TreeSHAP must be applied to a discriminative rather than a predictive classifier, and DSI must be defined dimensionality-invariant. The three contributions are: (1) an Adaptive Sequential Permutation Test (ASPT) with prediction entropy monitoring ($H_{\text{pred}}$) for statistically rigorous, computationally efficient drift confirmation with early-warning signals; (2) SHAP interaction-aware drift localisation and a novel Drift Severity Index (DSI) for correlated feature diagnosis and continuous severity quantification; and (3) a comprehensive experimental evaluation benchmarking EADD against seven state-of-the-art detectors. The experimental evaluation demonstrates that EADD is the only detector to achieve complete synthetic drift-type coverage (4/4) with zero false alarms, while also detecting drift on 3 of 4 annotated real-world datasets with competitive detection delays. By answering ``Is drift emerging?'', ``Is it statistically significant?'', ``What caused it?'', and ``How severe is it?'' in a unified framework, EADD provides a practical diagnostic foundation for production MLOps.
