...
\begin{figure}[!htpb]
    ...
    \caption{Detailed architecture of the Enhanced Adversarial Drift Detection (EADD) methodology. The framework highlights three novel contributions (blue boxes): (1) Prediction Entropy monitoring ($H_{\text{pred}}$) and Adaptive Sequential Permutation Testing (ASPT) for statistically rigorous drift confirmation with early-warning signals; (2) SHAP Interaction-Aware Localisation and Drift Severity Index (DSI) for feature-level diagnosis and severity quantification; and (3) comprehensive experimental evaluation. This pipeline transforms standard discriminative detection into an actionable MLOps monitoring system.}
...
\textbf{Step 2b: Prediction Entropy ($H_{\text{pred}}$)} --- Entropy-based early-warning signal (Novel Contribution~1)
...
\subsection{Step 2b: Prediction Entropy Monitoring ($H_{\text{pred}}$) --- Novel Contribution 1}
...
\begin{enumerate}
    ...
    \item \textbf{No Universally Dominant Detector:} No single detector achieved the best performance across all datasets, confirming the no-free-lunch characterization of the drift detection problem noted by \citet{Lukats2024}. The current runs show that EADD's attribution capability remains a core strength, but detection sensitivity requires recalibration for real-world streams.
\end{enumerate}
...
\begin{table}[!htbp]
\centering
\caption{Experiment 3: SHAP Feature Attribution Accuracy. P@k = Precision at $k$ (fraction of top-$k$ features that are truly drifted); R@k = Recall at $k$ (fraction of drifted features in top-$k$); F1@k = harmonic mean. DSI = AUC $\times$ (1 $-$ $H_{\text{norm}}(\mathbf{s})$); higher DSI indicates more concentrated, severe drift.}
...
\hline
Univariate (F3) & 0.784 & 1.0 & 1.0 & 1.0 & F3 (52.3\%) & 0.191 & Univariate $\checkmark$ \\
Subset (F2,F5,F7) & 0.809 & 1.0 & 1.0 & 1.0 & F5 (24.5\%) & 0.108 & Subset $\checkmark$ \\
Multivariate (all) & 0.806 & 1.0 & 1.0 & 1.0 & F2 (14.6\%) & 0.004 & Multivariate $\checkmark$ \\
\hline
\end{tabular}
\end{table}

\textbf{DSI Ordering:} The DSI values confirm the expected pattern: univariate drift (concentrated in one feature, F3 at 52.3\%) yields the highest DSI (0.191), subset drift (spread across three features) yields an intermediate value (0.108), and multivariate drift (spread uniformly across all 10 features) yields near-zero DSI (0.004). This monotonic decrease validates the metric's theoretical design. Note: at $d=10$, even a dominant single feature produces moderate $H_{\text{norm}}$ due to the remaining nine features' non-zero contributions; operating-point recalibration of DSI tiers for a specific dimensionality is advisable in practice.
...
The current evidence supports EADD's strength in false-alarm suppression and feature attribution accuracy, but not universal sensitivity. This suggests that the LightGBM + ASPT stack needs operating-point recalibration (windowing, thresholding, and significance settings) to maintain detection power on more challenging benchmark streams.
...
This thesis has advanced the discriminative drift detection paradigm through four specific methodological contributions: prediction entropy monitoring ($H_{\text{pred}}$) for early-warning signals, adaptive sequential permutation testing for computationally efficient significance calibration, SHAP interaction-aware drift localisation for correlated feature diagnosis, and the Drift Severity Index for continuous, theoretically motivated severity quantification. The latest experimental evaluation confirms EADD's strong false-alarm suppression and high-quality attribution in controlled settings, while also revealing conservative detection behavior in multi-detector benchmark configurations. By answering ``Is drift emerging?'', ``Is it statistically significant?'', ``What caused it?'', and ``How severe is it?'' in a unified framework, EADD provides a practical diagnostic foundation for production MLOps, with calibration identified as the primary next step for broader sensitivity.
