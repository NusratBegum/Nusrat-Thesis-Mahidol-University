...
\begin{enumerate}
    ...
    \item Introduce the \textbf{Prediction Uncertainty Index (PUI)}, a novel early-warning metric derived from the Shannon entropy \citep{Shannon1948} of the discriminative classifier's predictions, capable of flagging subtle distributional instabilities before they pass statistical significance thresholds.
    ...
    \item Systematically benchmark EADD against seven state-of-the-art unsupervised drift detectors on real-world datasets from the \citet{Lukats2024} benchmark, evaluating detection accuracy, false alarm control, and the practical value of explainability.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Prediction Uncertainty Index (PUI):} A novel uncertainty-aware monitoring signal that tracks the entropy of the discriminative classifier's predictions. PUI provides a continuous measure of distributional instability, serving as an ``early warning'' for subtle shifts that have not yet reached statistical significance ($p < 0.05$) but indicate increasing system entropy.
...
\begin{figure}[!htpb]
    ...
    % Novel 1: PUI
    \node[novel, above=1.5cm of c2st] (pui) {\textbf{Contribution 1}\\Prediction Uncertainty\\Index (PUI)};
    ...
    \caption{Detailed architecture of the Explainable Adversarial Drift Detection (EADD) methodology. The framework highlights four specific novel contributions (blue boxes): Prediction Uncertainty Index for early warning, Adaptive Sequential Permutation Testing for statistical rigor, SHAP Interaction-Aware Localization for root-cause diagnosis, and Drift Severity Index for severity-proportional remediation. This pipeline transforms standard discriminative detection into a continuous, actionable MLOps monitoring system.}
...
\begin{figure}[!htpb]
    ...
    \node[math_process, below=1.2cm of c2st, xshift=-4.5cm] (pui) {
        \textbf{Contribution 1: Prediction Uncertainty Index (PUI)} \\
        $\text{PUI} = H(\hat{p}) = \frac{-1}{|D|} \sum_{i \in D} [\hat{p}_i \ln \hat{p}_i + (1 - \hat{p}_i) \ln (1 - \hat{p}_i)]$ \\
...
\begin{enumerate}
    \item \textbf{Prediction Uncertainty Index (PUI):} A novel early-warning metric derived from the entropy of the discriminative classifier's predictions, capable of flagging subtle distributional instabilities before they pass statistical significance thresholds.

    \item \textbf{Adaptive Sequential Permutation Testing (ASPT):} An early-stopping permutation test \citep{Gandy2009} that terminates when evidence for or against drift is sufficient, reducing computational cost by up to 70\% compared to the fixed $B=50$ approach while maintaining drift confirmation at the nominal $\alpha=0.05$ level.
...
After training the discriminative classifier (Step~2), EADD computes the \textbf{Prediction Uncertainty Index (PUI)}---a novel contribution of this thesis---as an early-warning signal for distributional instability. PUI is derived from the binary entropy \citep{Shannon1948} of the C2ST discriminator's predictions:

\begin{equation}
\text{PUI} = \frac{-1}{n} \sum_{i=1}^{n} \left[\hat{p}_i \ln \hat{p}_i + (1 - \hat{p}_i) \ln (1 - \hat{p}_i)\right]
\label{eq:pui}
\end{equation}
...
\begin{itemize}
    \item $\text{PUI} \approx \ln 2$: Maximum uncertainty; the classifier cannot distinguish windows $\rightarrow$ no distributional difference
    \item $\text{PUI} \rightarrow 0$: High certainty; the classifier clearly separates windows $\rightarrow$ strong drift signal
    \item $0.35 < \text{PUI} < 0.6$: Intermediate uncertainty; subtle distributional change that may not yet reach statistical significance
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Initialise:} Set maximum budget $B_{\max} = 50$, nominal level $\alpha = 0.05$, and minimum sequential budget $B_{\min}=10$.
...
\textbf{Computational Savings:} In our experiments (Chapter~\ref{chap:results}), we logged the number of permutations executed per ASPT invocation. The empirically measured mean was substantially below the fixed budget in many no-drift cases due to Rule~3 (early acceptance), while drift cases often consumed a larger fraction of the budget. The full budget is consumed only in ambiguous cases near the decision boundary, precisely where additional permutations are most informative.
...
EADD first applies \textbf{TreeSHAP} (SHapley Additive exPlanations for tree ensembles) \citep{Lundberg2017, LundbergInteraction2020} to the discriminative classifier. TreeSHAP computes exact Shapley values \citep{Shapley1953} in polynomial time for tree-based models:
\begin{enumerate}
...
\textbf{Note on experimental protocols.} Two synthetic experiments use the same drift streams but differ in EADD configuration. Experiment~1 (Section~\ref{sec:exp1}) runs EADD in a focused two-detector temporal-pattern protocol with a tuned threshold, where EADD detects all four drift types (4/4, zero false alarms). Experiment~5, reported in the hypothesis-testing section (Section~\ref{sec:h1}), reruns the same streams in the full eight-detector default benchmark configuration, where EADD's stricter ASPT filter produces 0/4 detections---exposing a sensitivity--specificity trade-off under default settings that motivates the calibration work identified as future research.
...
\begin{enumerate}
    \item \textbf{Divergent Behavior Across Setups:} In the two-detector temporal-pattern protocol (Experiment 1), EADD detected all four drift types (100\% success) with zero false alarms. However, in the multi-detector synthetic benchmark (Experiment 5), EADD recorded no detections under the default configuration while several baselines (BNDM, CSDDM, IBDD, OCDD) still detected all four types.
    ...
    \item \textbf{Speed--Precision Trade-off:} IBDD remained the fastest detector (MTD of 11--18 samples in recurring/abrupt settings) but at an extreme false alarm cost (41--49 per drift type), rendering it impractical for production monitoring where each false alarm triggers unnecessary investigation or retraining.
\end{enumerate}

Overall, the updated synthetic results indicate strong EADD behavior in the focused temporal-pattern protocol, but reduced sensitivity in the broader multi-detector default-configuration benchmark. This cross-protocol mismatch is carried forward as a key limitation in later chapters.
...
\hline
Electricity & 0 & 37 & 40 & 79 & 726 & 37 & 75 \\
IncrAbrupt & 0 & 0 & 1$^*$ & 15 & 94 & 79 & 0 \\
Incremental & 0 & 0 & 1$^*$ & 19 & 63 & 79 & 1 \\
Reoccurring & 0 & 2 & 3$^*$ & 21 & 107 & 79 & 1 \\
NOAAWeather & 0 & 29 & 36 & 70 & 485 & 71 & 2 \\
OutdoorObjects & 0 & 1 & 8 & 15 & 34 & 15 & 14 \\
Powersupply & 0 & 11 & 16 & 58 & 163 & 79 & 9 \\
\hline
\textbf{Total} & \textbf{0} & \textbf{80} & \textbf{105$^*$} & \textbf{277} & \textbf{1{,}672} & \textbf{439} & \textbf{102} \\
\hline
...
\begin{figure}[!htbp]
    ...
    \end{minipage}
    \caption{Time-series comparison on real-world datasets. Left: WaveformDrift2---D3 and several baselines detect the annotated drift points, while EADD remains below detection threshold in the current configuration. Right: SineClusters---IBDD detects rapidly but with many excess detections, illustrating the sensitivity--specificity trade-off.}
...
\begin{enumerate}
    \item \textbf{Conservative EADD Operating Point:} Under the current real-world benchmark configuration, EADD recorded zero detections across all datasets. This behavior eliminates false positives in unknown-ground-truth streams, but also misses known drift points in annotated datasets.
    ...
    \item \textbf{InsectsGradual Challenge:} InsectsGradual remained one of the hardest datasets: EADD, BNDM, OCDD, and UDetect missed the annotated drift point. D3 triggered multiple detections but remained unstable on this stream, while CSDDM, IBDD, and SPLL detected it at the cost of higher detection counts on other datasets.
    
    \item \textbf{No Universally Dominant Detector:} No single detector achieved the best performance across all datasets, confirming the no-free-lunch characterization of the drift detection problem noted by \citet{Lukats2024}. The current runs show that EADD's explainability remains a core strength, but detection sensitivity requires recalibration for real-world streams.
\end{enumerate}
...
Heteroscedastic & 0.0 & 10.4 & 0.0 & 0.0 \\
Correlated & 0 & 11.0 & 0.2 & 0 \\
\hline
\textbf{Total} & \textbf{0.0} & \textbf{118.2} & \textbf{54.6} & \textbf{13.8} \\
\hline
\end{tabular}
\end{table}

\textbf{Key Findings:}
EADD produced zero false alarms across all four stable stream types in this run. D3's false alarm rates remained substantially higher---particularly on autocorrelated data, where D3 at $\tau=0.6$ triggered 87.4 mean false alarms and at $\tau=0.7$ produced 54.6 false alarms. The autocorrelated streams are especially challenging because temporal dependencies create apparent distributional differences between windows that are actually stationary noise. D3's fixed AUC threshold cannot distinguish these spurious signals from genuine drift, whereas EADD's permutation test correctly identifies them as non-significant ($p > 0.05$). Even at D3's most conservative threshold ($\tau=0.8$), autocorrelated data still produced 13.8 false alarms. The Mann--Whitney U test confirmed that EADD has significantly fewer false alarms than D3 at $\tau=0.6$ ($p = 0.0101$).
...
In this multi-detector synthetic benchmark, EADD did not trigger on any of the four drift types. Complete coverage (4/4) was achieved by BNDM, CSDDM, IBDD, and OCDD, each with higher false alarm counts. D3 and SPLL each cover 2/4 types (failing on gradual and incremental drift), while UDetect fails on all four types.

\textbf{H1b: Detection Delay Comparison}

For the two drift types where D3 and SPLL succeeded (abrupt and recurring), per-drift-type MTD comparisons showed faster response from IBDD and D3 but with substantial false alarm differences across detectors. Under the current default configuration, EADD was over-conservative in this benchmark.
...
The multi-detector comparison (Experiment 5) reinforces detector trade-offs: IBDD produced the fastest response but very high false alarms, while EADD remained conservative and did not trigger under the default benchmark setup.
...
\begin{figure}[!htbp]
    ...
    \caption{Overall summary comparison of all eight detectors across key performance dimensions. The latest runs highlight a stability--sensitivity trade-off, with EADD remaining highly stable but conservative under default multi-detector settings.}
...
\hline
1 & Synthetic & Detection across 4 drift types (8 detectors) & EADD: 0/4 types in multi-detector default setup; D3/SPLL: 2/4; BNDM/CSDDM/IBDD/OCDD: 4/4 with higher FA. \\
2 & Real-world & 11 datasets, 8 detectors & EADD: 0 detections in current run. D3/BNDM/CSDDM/IBDD/OCDD/SPLL remain active; BNDM timeout on 33d. \\
3 & Synthetic & SHAP attribution accuracy & P@k=R@k=F1@k=1.0 (all 3 scenarios). Prescription 3/3 correct. \\
4 & Synthetic (stable) & False alarm suppression & EADD: 0.0 mean FA (0 on all 4 types). D3: 87.4 FA on autocorrelated data at $\tau=0.6$. \\
\hline
\end{tabular}
\end{table}
...
\hline
H1a: Drift type coverage & Binomial test & 0.6875 & Not significant \\
H1b: Detection delay & Mann--Whitney U & 0.3333 & Not significant \\
H2: FA vs D3($\tau$=0.7) & Mann--Whitney U & 0.2266 & Not significant \\
H2: FA vs D3($\tau$=0.6) & Mann--Whitney U & \textbf{0.0101} & \textbf{Supported} \\
H3: SHAP attribution & Qualitative + P@k/R@k/F1@k & --- & Partial overall; perfect in controlled scenarios \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Synthetic Data (Experiment 1 vs Experiment 5):} Results diverged by protocol. In the focused two-detector temporal-pattern setting (Experiment 1), EADD achieved complete coverage (4/4 types) with zero false alarms. In the multi-detector default benchmark (Experiment 5), EADD did not trigger, while BNDM, CSDDM, IBDD, and OCDD detected all four types with higher false alarm counts.
    
    \item \textbf{Real-World Data (Experiment 2/6):} Across 11 real-world datasets, EADD remained highly conservative and produced zero detections in the current run. Other detectors remained active (for example, D3 and SPLL on known-drift datasets), while BNDM timed out on all 33-feature datasets, revealing a scalability limitation. IBDD again produced very high detection counts (1{,}672 on no-ground-truth datasets), indicating over-sensitivity.
    
    \item \textbf{Key Limitation:} Under default benchmark settings, EADD appears over-conservative in both synthetic multi-detector and real-world multi-detector evaluations, suggesting that calibration settings should be tuned before deployment.
\end{itemize}

The current evidence supports EADD's strength in false-alarm suppression and explainability, but not universal sensitivity. This suggests that the LightGBM + ASPT stack needs operating-point recalibration (windowing, thresholding, and significance settings) to maintain detection power on more challenging benchmark streams.
...
\begin{itemize}
    ...
    \item \textbf{False Alarm Discipline on Drift Data (Experiment 5):} Multi-detector synthetic results continued to show high false-alarm costs for highly sensitive detectors (e.g., IBDD, OCDD), while EADD remained conservative and did not trigger.
    
    \item \textbf{Real-World Selectivity (Experiment 2):} On 7 no-ground-truth datasets, EADD produced 0 detections in the current run. IBDD produced 1{,}672, OCDD 439, CSDDM 277, and SPLL 102; D3 produced 80.
...
\textbf{Covariate Drift Only:} EADD detects feature drift ($P(X)$ changes) but cannot directly detect pure concept drift ($P(y|X)$ changes with stable $P(X)$). The current runs also indicate conservative behavior that can suppress true positives under some configurations. The hierarchical decomposition of \citet{HDPD2024} addresses this gap but requires labelled data.
...
\textbf{Sensitivity Calibration:} EADD missed annotated drifts in the current multi-detector benchmark configuration, suggesting that windowing and decision thresholds require further calibration for real-world conditions.
...
The experimental results provide mixed evidence across research questions. EADD consistently demonstrated strong false-alarm suppression and explainability in controlled settings (RQ2 and RQ3), including zero false alarms on stable streams and perfect P@k/R@k/F1@k in Experiment 3. However, cross-benchmark sensitivity was inconsistent: EADD performed strongly in focused temporal-pattern runs but remained over-conservative in multi-detector benchmark settings, motivating additional calibration work for RQ1. Hypothesis H2 was confirmed with statistical significance ($p = 0.0101$ vs D3 at $\tau=0.6$), while H1 and H3 were not uniformly supported across all evaluation contexts.
...
\begin{itemize}
    \item \textbf{Experiment 1/5:} Synthetic drift evaluation showed protocol-dependent outcomes: EADD was successful in focused temporal-pattern testing but conservative in the multi-detector default benchmark, while BNDM/CSDDM/IBDD/OCDD retained broad sensitivity at higher false alarm cost.
    \item \textbf{Experiment 2/6:} In the 11-dataset real-world benchmark, EADD produced no detections in the current configuration, while several baselines remained active. BNDM timed out on high-dimensional streams; IBDD produced 1{,}672 excess detections.
...
This thesis has advanced the discriminative drift detection paradigm through four specific methodological contributions: the Prediction Uncertainty Index for entropy-based early warning, adaptive sequential permutation testing for computationally efficient significance calibration, SHAP interaction-aware drift localisation for correlated feature diagnosis, and the Drift Severity Index for continuous, theoretically motivated severity quantification. The latest experimental evaluation confirms EADD's strong false-alarm suppression and high-quality attribution in controlled settings, while also revealing conservative detection behavior in multi-detector benchmark configurations. By answering ``Is drift emerging?'', ``Is it statistically significant?'', ``What caused it?'', and ``How severe is it?'' in a unified framework, EADD provides a practical diagnostic foundation for production MLOps, with calibration identified as the primary next step for broader sensitivity.
