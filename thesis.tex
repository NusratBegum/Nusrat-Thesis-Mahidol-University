...
\begin{enumerate}
    ...
    \item Systematically benchmark EADD against state-of-the-art unsupervised drift detectors---including D3 \cite{Gozuacik2019}, ADWIN \cite{BifetGavalda2007}, KSWIN \cite{Raab2020}, and MMD \cite{Gretton2012}---on the real-world datasets from the \citet{Lukats2025} benchmark.
...
\begin{enumerate}
    ...
    \item \textbf{No Root Cause Analysis:} Current methods provide only a binary detection signal without identifying the specific source of the change \citep{Lukats2025}. Production systems require diagnostic capabilities that pinpoint drift sources and guide remediation.
...
While existing methods like the Discriminative Drift Detector (D3) \citep{Gozuacik2019} successfully employ adversarial validation to flag distributional changes, they function as opaque detectors---providing a drift signal without explaining \textit{which} features drifted. Conventional drift detection methods produce only binary alerts; EADD is designed as a diagnostic framework that identifies exactly which features contributed to the drift, enabling targeted corrective action.
...
A raw AUC score (e.g., 0.65) is inconclusive, as it may reflect genuine distributional change or statistical noise. Standard adversarial validation \citep{Pan2020} relies on fixed thresholds (e.g., AUC $> 0.7$), which lack principled justification. EADD addresses this limitation with a \textbf{Permutation Test} \citep{LopezPazOquab2017}.
...
\begin{itemize}
    \item \textbf{False Alarm Elimination:} EADD produced zero false alarms across all four stable stream types and all runs, compared to D3's substantial false alarm rates---particularly 87.4 mean false alarms on autocorrelated data at $\tau=0.6$, and 54.6 even at $\tau=0.7$.
...
This thesis addressed a critical gap in modern drift detection: the lack of explainability. While state-of-the-art methods like the Discriminative Drift Detector (D3) \citep{Gozuacik2019} have demonstrated that adversarial validation can accurately detect distribution changes, they do not explain \textit{why} the data changed or \textit{what} corrective action should be taken.
