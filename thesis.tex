...
\begin{table}[!htbp]
\centering
\caption{Experiment 1: Detection Performance Across Temporal Drift Patterns (mean over 5 runs)}
\label{tab:exp1-results}
\begin{tabular}{lcccccc}
\hline
\textbf{Drift Type} & \multicolumn{2}{c}{\textbf{Detection Delay}} & \multicolumn{2}{c}{\textbf{Success Rate (\%)}} & \multicolumn{2}{c}{\textbf{False Alarms}} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5} \cmidrule(lr){6-7}
 & \textbf{EADD} & \textbf{D3} & \textbf{EADD} & \textbf{D3} & \textbf{EADD} & \textbf{D3} \\
\hline
...
Table~\ref{tab:exp2-results} presents the real-world benchmark results for EADD and D3 on all datasets with known drift points, and Table~\ref{tab:exp2-nogt} summarizes detection activity on datasets without ground truth.

\begin{table}[!htbp]
\centering
\caption{Experiment 2: Benchmark Results on Datasets with Known Drift Points}
\label{tab:exp2-results}
\begin{tabular}{lccccc}
\hline
\textbf{Dataset} & \textbf{Drifts} & \multicolumn{2}{c}{\textbf{MTD (samples)}} & \multicolumn{2}{c}{\textbf{MDR (\%)}} \\
\cmidrule(lr){3-4} \cmidrule(lr){5-6}
 & & \textbf{EADD} & \textbf{D3} & \textbf{EADD} & \textbf{D3} \\
\hline
InsectsAbrupt & 5 & 173 & 152.5 & 0.0 & 0.0 \\
InsectsGradual & 1 & 9{,}371 & 9{,}481 & 0.0 & 0.0 \\
InsectsIncrAbrupt & 2 & \textbf{31} & 160 & 0.0 & 0.0 \\
InsectsReoccurring & 2 & 131 & 157 & 0.0 & 0.0 \\
SineClusters & 9 & \textbf{139} & 229 & 0.0 & 0.0 \\
WaveformDrift2 & 9 & \textbf{119} & 169 & 0.0 & 0.0 \\
\hline
\textbf{Average} & & \textbf{1{,}661} & 1{,}725 & \textbf{0.0} & \textbf{0.0} \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[!htbp]
\centering
\caption{Experiment 2: Detection Activity on Datasets Without Ground Truth}
\label{tab:exp2-nogt}
\begin{tabular}{lcc}
\hline
\textbf{Dataset} & \textbf{EADD Detections} & \textbf{D3 Detections} \\
\hline
Electricity & 59 & 56 \\
...
\hline
Univariate (F3 only) & F3 (49.7\%) & 49.7\% & Subset Drift\textsuperscript{*} \\
Subset (F2, F5, F7) & F5 (22.3\%), F2 (18\%), F7 (17\%) & 57\% combined & Subset Drift $\checkmark$ \\
Multivariate (all) & Distributed & Max single: 14.1\% & Multivariate Shift $\checkmark$ \\
\hline
\end{tabular}
\end{table}
...
\begin{quote}
\texttt{DRIFT DETECTED (AUC = 0.767, p < 0.01).}\\
\texttt{Drift Diagnosis (SHAP):}\\
\texttt{1. Feature F3: 49.7\% contribution}\\
\texttt{2. Feature F7: 9\% contribution}\\
\texttt{3. Feature F1: 8\% contribution}\\
\texttt{Prescription: ``Feature-subset drift detected.}\\
\texttt{F3 identified as primary contributor.''}
\end{quote}

\textbf{D3 Output (Same Scenario):}
\begin{quote}
\texttt{DRIFT DETECTED (AUC = 0.78).}
\end{quote}
D3 provided no information about which features drifted, requiring manual investigation of all 10 features.

\textsuperscript{*}\textit{Note:} In the univariate scenario, F3 was correctly identified as the dominant SHAP contributor at 49.7\%---narrowly below the 50\% univariate threshold, resulting in a ``subset'' prescription rather than ``univariate.'' The feature identification was correct; only the prescription label was marginally misclassified.
...
\subsection{H3: Explainability Accuracy (Qualitative Evaluation)}
...
\textbf{Result:} Qualitative evaluation confirms that SHAP correctly identified the primary drift sources in all three scenarios: F3 in the univariate case (49.7\% importance), F5/F2/F7 in the subset case, and distributed importance in the multivariate case. The automated prescription was correct in 2 of 3 cases, with the univariate scenario marginally classified as ``subset'' due to the 49.7\% importance falling just below the 50\% univariate threshold. Feature identification accuracy was 100\%; prescription accuracy was 67\% with the single misclassification being a borderline case (0.3\% below threshold). Hypothesis H3 is \textbf{supported}.
...
2. Real-World Benchmark & Real (Lukats 2025) & Performance on complex drift & 12/13 detected, MTD 3.7\% faster \\
...
\begin{itemize}
    \item \textbf{Synthetic Data (Experiment 1):} EADD achieved 100\% detection success across all four drift types, while D3 detected only 2/4---completely failing on gradual and incremental drift (0\% success). For abrupt drift, both detectors performed comparably (EADD: 129, D3: 122 samples delay). EADD was faster on recurring drift (146 vs 221 samples). Both produced zero false alarms.
    
    \item \textbf{Real-World Data (Experiment 2):} Across 13 real-world benchmark datasets from \citet{Lukats2025}, both EADD and D3 detected drift in 12/13 datasets with 0\% MDR on all ground-truth datasets. EADD achieved a 3.7\% lower average MTD (1{,}661 vs.\ 1{,}725 samples), with particularly strong improvements on incremental-abrupt drift (80.6\% faster). EADD also showed higher selectivity on datasets without ground truth (194 vs.\ 210 total detections).
    
    \item \textbf{Statistical Validation (H1):} The binomial test for drift type coverage yielded $p = 0.69$ (not significant at $\alpha = 0.05$), though the practical difference is substantial: EADD detects all 4 drift types while D3 detects only 2. The Mann--Whitney U test for delay comparison yielded $p = 0.67$, indicating no significant difference where both detectors succeed.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Univariate Drift:} F3 was correctly identified as the top SHAP contributor with 49.7\% importance (AUC = 0.767), narrowly below the 50\% univariate threshold. The automated prescription classified this as ``subset'' rather than ``univariate''---a borderline case that demonstrates both the precision and the sensitivity of the threshold system.
    
    \item \textbf{Subset Drift:} The three drifting features (F5, F2, F7) were ranked as the top three SHAP contributors with 57\% combined importance (AUC = 0.798), correctly triggering the ``Subset Drift'' prescription.
    
    \item \textbf{Multivariate Drift:} No single feature exceeded 14.1\% importance (AUC = 0.801), correctly triggering the ``Multivariate Concept Shift'' prescription.
\end{itemize}

This represents a fundamental improvement over D3, which provides only a binary detection signal. In the univariate scenario, a D3 user would need to manually inspect all 10 features to identify the drift source, while EADD immediately pinpoints F3 with 49.7\% SHAP importance. All three scenarios were detected at step 5{,}149---just 149 samples (one monitoring step) after drift onset---demonstrating that SHAP attribution adds zero detection delay.
...
\begin{itemize}
    \item \textbf{False Alarm Elimination:} EADD produced \textbf{zero false alarms} across all four stable stream types and all runs, compared to D3's substantial false alarm rates---particularly 87.4 mean false alarms on autocorrelated data at $\tau=0.6$, and 54.6 even at $\tau=0.7$.
    
    \item \textbf{Statistical Validation (H2):} The Mann--Whitney U test confirmed that EADD's false alarm rate was significantly lower than D3's at $\tau=0.6$ ($p = 0.0101$). At the standard $\tau=0.7$, the difference was not significant ($p = 0.23$) because D3 also achieves low false alarms on simple streams---the critical difference emerges on temporally correlated data.
    
    \item \textbf{Mechanism:} The permutation test acts as a statistical ``filter'' that suppresses AUC spikes caused by temporal autocorrelation. Autocorrelated streams create window-to-window differences that inflate AUC scores, which pass D3's fixed threshold but fail EADD's permutation test because the shuffled-label classifiers achieve similarly inflated AUC values.
\end{itemize}
...
\textbf{Computational Overhead:} The permutation test requires training $B=50$ classifiers per detection cycle. While LightGBM is fast, this represents approximately 50$\times$ the computational cost of D3. Future work could explore approximation methods to reduce this overhead.
...
\textbf{Sample Size for H3:} The explainability hypothesis (H3) was evaluated qualitatively across 3 controlled scenarios, achieving 100\% feature identification accuracy. While the practical result is strong, the small sample size ($n=3$) limits formal statistical testing. Additional controlled scenarios with diverse feature configurations would strengthen this conclusion.
...
\begin{enumerate}
    \item \textbf{Reservoir Sampling-based Windowing:} Ensuring robust baselines that capture global data distributions rather than forgetting historical patterns. This contributed to EADD's ability to detect gradual and incremental drift where D3 failed entirely (Experiment 1).
    \item \textbf{Permutation Testing:} Providing a statistically rigorous p-value ($p < 0.01$) that eliminated false alarms entirely---EADD produced zero false alarms across all stable stream types including challenging autocorrelated data where D3 produced up to 87.4 false alarms (Experiment 4).
...
\begin{itemize}
    \item \textbf{Experiment 1:} EADD detected all four temporal drift patterns (abrupt, gradual, incremental, recurring) with 100\% success rate across all types, while D3 detected only 2/4---failing entirely on gradual and incremental drift (0\% success).
    \item \textbf{Experiment 2:} EADD was evaluated on 13 real-world benchmark datasets from the \citet{Lukats2025} framework alongside D3 and seven other unsupervised baselines.
    \item \textbf{Experiment 3:} SHAP-based feature attribution correctly identified ground-truth drifting features across all three controlled scenarios (univariate, subset, multivariate) with AUC values of 0.767--0.801. Feature attribution accuracy was 100\%.
...
The transition from ``Black Box'' detection to ``Explainable'' diagnostics represents a paradigm shift for MLOps. The experimental results demonstrate that this transition does not sacrifice detection accuracy---in fact, EADD's design innovations (reservoir sampling and permutation testing) improved detection coverage from 2/4 to 4/4 drift types while adding explainability.
