...
\begin{figure}[!htpb]
    ...
    % Step 2: Discriminative Classification (C2ST)
    \node[standard, right=of window] (adv) {Step 2:\\Discriminative\\Classification};
...
After training the discriminative classifier (Step~2), EADD computes the \textbf{Prediction Uncertainty Index (PUI)}---a novel contribution of this thesis---as an early-warning signal for distributional instability. PUI is derived from the normalised binary entropy \citep{Shannon1948} of the C2ST discriminator's predictions:
...
\textbf{Computational Savings:} In our experiments (Chapter~\ref{chap:results}), we logged the number of permutations executed per ASPT invocation. The empirically measured mean was 12.3 permutations when drift was present (early rejection via Rule~2) and 9.7 when absent (early acceptance via Rule~3), representing a 75--81\% reduction in computational cost compared to fixed $B=50$. This is consistent with the theoretical analysis by \citet{Gandy2009}, who proved that early stopping maintains Type~I error control under the bounded resampling risk condition. The full budget is consumed only in ambiguous cases near the decision boundary, precisely where additional permutations are most informative.
