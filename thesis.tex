...
\begin{enumerate}
    ...
    \item Develop an \textbf{adaptive sequential permutation test (ASPT)} that calibrates the number of permutations based on accumulated evidence strength, reducing the permutation budget by 40--80\% in practice compared to fixed-budget approaches while maintaining Type~I error control at the nominal level \citep{Gandy2009}.
...
\begin{table}[!htbp]
\centering
\caption{Experiment 2: EADD vs.\ D3 on the four annotated real-world datasets. Rec.\ = Recall on known drift points. MTD = Mean Time to Detection (samples; lower is better). Bold indicates EADD outperforms D3. ``---'' = all known drift points missed. Data from \texttt{experiment2\_realworld.csv}.}
\label{tab:exp2-results}
\begin{tabular}{l|c|c|c|c}
\hline
\textbf{Dataset} & \textbf{Rec.\ (EADD)} & \textbf{Rec.\ (D3)} & \textbf{MTD (EADD)} & \textbf{MTD (D3)} \\
\hline
...
\begin{figure}[!htbp]
    ...
    \end{minipage}
    \caption{Time-series comparison on benchmark datasets from the multi-detector supplementary run. Left: WaveformDrift2 (semi-synthetic)---EADD detects all three drift points (MTD=99), outperforming D3 (MTD=179). Right: SineClusters (semi-synthetic)---IBDD detects rapidly but with many excess detections, illustrating the sensitivity--specificity trade-off.}
...
\begin{enumerate}
    \item Inspired by the statistical analysis of C2ST \citep{LopezPazOquab2017} and its extensions \citep{EC2ST2024}: an \textbf{Adaptive Sequential Permutation Test (ASPT)} \citep{Gandy2009} with \textbf{prediction entropy monitoring} ($H_{\text{pred}}$). ASPT monitors evidence accumulation and terminates when the decision boundary is crossed, reducing the permutation budget by 40--80\% in practice while maintaining Type~I error control. $H_{\text{pred}}$ flags emerging distributional instabilities before AUC exceeds the significance threshold.
