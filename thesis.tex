...
\begin{enumerate}
    ...
    \item Develop an \textbf{adaptive sequential permutation test (ASPT)} that calibrates the number of permutations based on accumulated evidence strength, reducing the permutation budget by 60--80\% in practice compared to fixed-budget approaches while maintaining Type~I error control at the nominal level \citep{Gandy2009}.
...
\begin{enumerate}
    \item Inspired by the statistical analysis of C2ST \citep{LopezPazOquab2017} and its extensions \citep{EC2ST2024}: an \textbf{Adaptive Sequential Permutation Test (ASPT)} \citep{Gandy2009} with \textbf{prediction entropy monitoring} ($H_{\text{pred}}$). ASPT monitors evidence accumulation and terminates when the decision boundary is crossed, reducing the permutation budget by 60--80\% in practice while maintaining Type~I error control. $H_{\text{pred}}$ flags emerging distributional instabilities before AUC exceeds the significance threshold.
...
\textbf{Computational Efficiency via ASPT:} The adaptive sequential permutation test reduces the mean permutation budget by 60--80\% compared to the fixed-budget baseline (averaging $\sim$12 of 50 maximum permutations on stable segments), making permutation-based statistical calibration practical for moderate-frequency monitoring. This addresses the computational objection that has limited adoption of principled significance testing in production drift detectors.
