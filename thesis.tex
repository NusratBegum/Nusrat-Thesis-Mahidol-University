...
\begin{itemize}
    \item \textbf{D3 (Discriminative Drift Detector)} \cite{Gozuacik2019}: Trains a logistic regression classifier to distinguish old from new data windows, using AUC as the drift signal. Identified as the overall best-performing unsupervised detector in the \citet{Lukats2024} benchmark.
    ...
    \item \textbf{IBDD (Image-Based Drift Detector)} \cite{IBDD2021}: Converts numerical data streams into images and applies image comparison techniques to detect distributional changes.
...
\begin{enumerate}
    ...
    \item \textbf{No Universally Dominant Detector:} No single detector achieved the best performance across all datasets, confirming the no-free-lunch characterization of the drift detection problem noted by \citet{Lukats2024}. However, EADD provides the best \textit{balance} of detection coverage, false alarm suppression, and---uniquely---explainability.
\end{enumerate}
